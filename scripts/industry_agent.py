...
def payload_for(spec,prompt):
    ...
    for p in spec.get('parameters',[]):
        ...
        elif lname in {'max_new_tokens','max_tokens','maximum_new_tokens'}: payload[name]=800
        elif lname=='temperature': payload[name]=0.2
        elif lname=='top_p': payload[name]=0.9
        elif lname=='top_k': payload[name]=40
        elif lname in {'system','system_prompt'}: payload[name]='Industrial engineering analysis. CLAIM<=EVIDENCE. SIMULATION!=TEST.'
...
mission='CEREBRON OMEGA FARM 16 INDUSTRY MANUFACTURING. Separate established engineering facts, assumptions, calculations, simulation proposals, test requirements, risks, unknowns and decisions. Do not fabricate measurements, plant data, standards compliance, supplier capability or experimental results. CLAIM<=EVIDENCE. SIMULATION!=TEST.'
prompt=f'{mission}\nROLE: {role}\nFOCUS: {focus}\nReturn findings, calculations_or_checks, failure_modes, validation_tests, unknowns, recommendations.'
ok,text,meta=invoke(model,prompt)
out={'role':role,'model':model,'focus':focus,'inference_success':bool(ok),'status':'UNREVIEWED_EXTERNAL_AGENT_OUTPUT' if ok else 'EXTERNAL_INFERENCE_FAILED','result':text if ok else None,'error':None if ok else meta.get('error'),'meta':meta}
pathlib.Path('out').mkdir(exist_ok=True)
pathlib.Path(f'out/{role}.json').write_text(json.dumps(out,ensure_ascii=False,indent=2),encoding='utf-8')
